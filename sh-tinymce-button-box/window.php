...
<script language="javascript" type="text/javascript" src="<?php echo $shtb_box_url; ?>tinymce.js?ver=0.7.5"></script>
<base target="_self" />
</head>
...
if ($shtb_adv_setting_opt['shtb_adv_using_syntaxhighlighter'] == 'wp_syntaxhighlighter' && function_exists('wp_sh_register_menu_item')) {
	...
	if (is_array($wp_sh_setting_opt)) {
		if ($wp_sh_setting_opt['gutter'] == "ture" || get_option('wp_sh_gutter') == 1) {
			...
			$shtb_adv_codebox_starting_linenumber_value = $shtb_adv_setting_opt['shtb_adv_first_line'];
		}
	} else {
		$shtb_adv_codebox_starting_linenumber_value = $shtb_adv_setting_opt['shtb_adv_first_line'];
