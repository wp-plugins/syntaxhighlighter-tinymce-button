...
<script language="javascript" type="text/javascript" src="<?php echo $shtb_box_url; ?>tinymce.js?ver=0.7.2"></script>
<base target="_self" />
</head>
	...
	<form name="shtb_adv_codebox" action="#">
		<table border="0" cellpadding="4" cellspacing="0">
			...
			<tr>
				...
				<td><?php $syntaxhighlighter_settings = get_option('syntaxhighlighter_settings'); if ($shtb_adv_setting_opt['shtb_adv_using_syntaxhighlighter'] == 'wp_syntaxhighlighter' && function_exists('wp_sh_register_menu_item') && get_option('wp_sh_gutter') == 1) {$shtb_adv_codebox_starting_linenumber_value = get_option('wp_sh_first_line');} else if ($shtb_adv_setting_opt['shtb_adv_using_syntaxhighlighter'] == 'syntaxhighlighter_evolved' && function_exists('SyntaxHighlighter') && $syntaxhighlighter_settings['gutter'] == 1) {$shtb_adv_codebox_starting_linenumber_value = $syntaxhighlighter_settings['firstline'];} else {$shtb_adv_codebox_starting_linenumber_value = $shtb_adv_setting_opt['shtb_adv_first_line'];}?><label><input name="shtb_adv_codebox_starting_linenumber" id='shtb_adv_codebox_starting_linenumber' type="text" value="<?php echo $shtb_adv_codebox_starting_linenumber_value; ?>" /></label></td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td><?php if ($shtb_adv_setting_opt['shtb_adv_html_script'] == "0") {$shtb_adv_codebox_html_script_check = ' ';} else {$shtb_adv_codebox_html_script_check = 'checked="checked" ';} ?><label><input name="shtb_adv_codebox_html_script" id='shtb_adv_codebox_html_script' type="checkbox" <?php echo $shtb_adv_codebox_html_script_check; ?>/></label></td>
			</tr>
			<tr>
